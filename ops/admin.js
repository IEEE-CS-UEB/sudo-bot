...
const isolateMemberFromMention = (
  ...
  try {
    if (time > 0 || time.isNumeric) {
      const user = botIO._bot.guilds
        .get(botIO._msg.guildID)
        .members.get(mention).username;
      console.log(user);
      const date = new Date(Date.now() + time * 60 * 1000);
      botIO._bot.guilds
        .get(botIO._msg.guildID)
        .editMember(mention, { communicationDisabledUntil: date });
      botIO.say(`Usuario ${user} esta aislado`);
    } else {
      botIO.say(`El tiempo esta mal ingresado debe ser numerico y mayor a 0`);
    }
  } catch (error) {
    botIO.say(`Error: no existe el usuario`);
  }
};
const outIsolateMemberFromMention = (
  ...
  try {
    ...
    botIO._bot.guilds
      .get(botIO._msg.guildID)
      .editMember(mention, { communicationDisabledUntil: null });
    botIO.say(`Usuario ${user} salio de la estar aislado`);
  } catch (e) {
    botIO.say(`Error: el usuario no puede ser aislado o no existe el usuario`);
  }
};
const muteMemberFromMention = (
  ...
  chan.editPermission(
    ...
    Eris.Constants.Permissions.none,
    1,
    "No reason provided"
  );
...
const muteMemberVoiceFromMention = (
  ...
  try {
    ...
    botIO._bot.guilds
      .get(botIO._msg.guildID)
      .editMember(mention, { mute: true });
    botIO.say(`Usuario ${user} muteado`);
  } catch (error) {
    botIO.say(`Error: no existe el usuario`);
  }
};
const unmuteMemberVoiceFromMention = (
  ...
  try {
    ...
    botIO._bot.guilds
      .get(botIO._msg.guildID)
      .editMember(mention, { mute: false });
    botIO.say(`Usuario ${user} ya puede hablar`);
  } catch (error) {
    botIO.say(`Error: no existe el usuario`);
  }
};
const unmuteMemberVoiceEvery = (
  ...
  try {
    ...
    for (let [key, value] of user) {
      botIO._bot.guilds
        .get(botIO._msg.guildID)
        .editMember(key, { mute: false });
    }

    botIO.say(`Todo los usuarios puede hablar`);
  } catch (error) {
    botIO.say(`Error: intente mas tarde`);
  }
};
const muteMemberVoiceEvery = (
  ...
  try {
    ...
    botIO.say(`Todo los usuarios estan muteados`);
  } catch (error) {
    botIO.say(`Error: intente mas tarde`);
  }
};
  ...
  try {
    ...
    for (let [key, value] of users) {
      ...
      if (key != mention) {
        botIO._bot.guilds
          .get(botIO._msg.guildID)
          .editMember(key, { mute: true });
      }
    }
    botIO.say(`Todo los usuarios estan muteados, menos ${user} `);
  } catch (error) {
    botIO.say(`Error: intente mas tarde`);
  }
};
module.exports = {
  outIsolateMemberFromMention,
  muteMemberVoiceEvery,
  muteMemberVoiceEveryBut,
  banMemberFromMention,
  unbanMemberFromQuery,
  muteMemberFromMention,
  isolateMemberFromMention,
  muteMemberVoiceFromMention,
  unmuteMemberVoiceFromMention,
  unmuteMemberVoiceEvery,
};
